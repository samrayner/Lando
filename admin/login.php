...
<form action="<?php echo $self ?>" method="post" id="login-form">
	<header>
		<h1 class="icon-lock">Login Required</h1>

		<div id="buttons">
			<a href="<?php echo $config["site_root"] ?>" class="button icon-home">Home</a>
		</div>
	</header>
	...
	<div id="login">
		...
		<?php
		if(isset($error))
				echo '<p class="icon-warning-sign notify failure">'.$error.'</p>';
		?>
		...
		<div id="login-button">
			<button class="button icon-key">Log In</button>
		</div>
